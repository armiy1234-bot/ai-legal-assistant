...
/**
 * Кастомный VK провайдер без PKCE (VK его не поддерживает)
 */
function CustomVKProvider(options: { clientId: string; clientSecret: string }) {
  return {
    id: "vk",
    name: "VK",
    type: "oauth" as const,
    ...
    checks: ["state"] as ("pkce" | "state" | "none")[], // Только state, без PKCE
    authorization: {
...
export const { handlers, auth, signIn, signOut } = NextAuth({
  ...
  debug: true, // Включаем дебаг для диагностики
});
